 ...
 */
class Product_Shortcode_2 extends Widget_Base {
	/**
	...
	public function get_name() {
		return 'durotan-product-shortcode-2';
	}
	...
	public function get_title() {
		return esc_html__( 'Durotan - Product Shortcode 2', 'durotan' );
	}
    ...
    protected function section_content_settings() {
		...
		$this->add_control(
			'show_product_taxonomy',
			[
				'label'       => esc_html__( 'Product Taxonomy', 'durotan' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => [
					'nne' 			=> esc_html__( 'None', 'durotan' ),
		...
		$this->add_control(
			'show_size_chart',
			[
				'label'     => esc_html__( 'Size Chart', 'durotan' ),
				'type'      => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'durotan' ),
				'label_on'  => __( 'Show', 'durotan' ),
				'return_value' => 'show',
		...
		$this->add_control(
			'show_buy_now',
			[
				'label'     => esc_html__( 'Buy now', 'durotan' ),
				'type'      => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'durotan' ),
				'label_on'  => __( 'Show', 'durotan' ),
				'return_value' => 'show',
	...
	protected function section_content_style() {
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .durotan-product-shortcode-2 .durotan-product-shortcode__title',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .durotan-product-shortcode-2 .durotan-product-shortcode__title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors'  => [
					'{{WRAPPER}} .durotan-product-shortcode-2 .durotan-product-shortcode__title' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .durotan-product-shortcode-2 .woocommerce-product-gallery__wrapper' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
        $this->add_control(
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .durotan-product-shortcode-2 .entry-summary' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->end_controls_section();
	...
	protected function render() {
		...
		echo '<div class="durotan-product-wrapper durotan-container">';
        ...
        if ( empty( $product ) ) {
			...
			if ( $settings['show_description'] != '' ) {
				add_action( 'durotan_woocommerce_single_product_summary', 'woocommerce_template_single_excerpt', 20 );
			}

			if ( $settings['show_meta'] != '' ) {
				add_action( 'durotan_woocommerce_single_product_summary', 'woocommerce_template_single_meta', 30 );
			}

			if ( $settings['show_size_chart'] != '' && class_exists('\Durotan\Addons\Modules\Size_Guide') ) {
				add_action( 'durotan_woocommerce_single_product_summary', array( \Durotan\Addons\Modules\Size_Guide::instance(), 'display_size_guide' ));
				add_action( 'durotan_woocommerce_after_single_product_summary', array( \Durotan\Addons\Modules\Size_Guide::instance(), 'size_guide_panel' ));
			}
