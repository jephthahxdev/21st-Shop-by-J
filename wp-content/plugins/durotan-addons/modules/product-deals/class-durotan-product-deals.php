...
class Product_Deals {
	...
	public function __construct() {
		...
		add_action( 'durotan_woocommerce_product_quickview_summary', array( $this, 'single_product_template' ), 75 );
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'durotan-deals', DUROTAN_ADDONS_URL . 'modules/product-deals/assets/deals.css', array(), '1.0.0' );
		wp_enqueue_script( 'coundown', DUROTAN_ADDONS_URL . '/assets/js/plugins/jquery.coundown.js', array(), '1.0', true );
	}
	...
	public static function single_product_template() {
		...
		$expire_date = apply_filters( 'durotan_product_deals_expire_timestamp', $expire_date, $product );
		...
		wc_get_template(
			...
			array(
				...
				'limit'           => get_post_meta( $product->get_id(), '_deal_quantity', true ),
				'sold'            => intval( get_post_meta( $product->get_id(), '_deal_sales_counts', true ) ),
				'expire_text'     => str_replace( '/', '<br>', get_option( 'durotan_product_deals_expire_text' ) ),
				'sold_items_text' => str_replace( '/', '<br>', get_option( 'durotan_product_deals_sold_items_text' ) ),
				'sold_text'       => get_option( 'durotan_product_deals_sold_text'),
				'countdown_texts' => \Durotan\Addons\Helper::get_countdown_texts()
			),
			'',
			DUROTAN_ADDONS_DIR . 'modules/product-deals/templates/'
		);
	}
}