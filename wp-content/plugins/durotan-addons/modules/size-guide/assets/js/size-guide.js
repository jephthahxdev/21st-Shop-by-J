jQuery(document).ready(function ($) {
	...
	// Render tabs.
	var $tabs = $( '#durotan-size-guide-tabs' ),
		$tabsNav = $( '.durotan-size-guide-tabs--tabs', $tabs ),
		$addTab = $( '.add-new-tab', $tabsNav ),
		templates = {
			tab: wp.template( 'durotan-size-guide-tab' ),
			panel: wp.template( 'durotan-size-guide-panel' )
		};

	if ( ! _.isUndefined( durotanSizeGuideTables ) ) {
		for ( var i = 0; i < durotanSizeGuideTables.tables.length; i++ ) {
			$addTab.before( templates.tab( {
				index: i,
				tab: durotanSizeGuideTables.tabs[i]
			} ) );

			$tabs.append( templates.panel( {
				index: i,
				name: durotanSizeGuideTables.names[i],
				description: durotanSizeGuideTables.descriptions[i],
				table: durotanSizeGuideTables.tables[i],
				information: durotanSizeGuideTables.information[i]
			} ) );

			$tabs.find( '.durotan-size-guide-table-editor[data-tab='+ i +']' ).find( '.durotan-size-guide-table' ).durotanEditTable();

			$addTab.data( 'index', i );

			if ( 0 === i ) {
				$tabsNav.children( ':eq(0)' ).addClass( 'active' );
				$tabs.find( '.durotan-size-guide-table-editor[data-tab='+ i +']' ).addClass( 'active' );
			}
	...
	// Change active tab.
	$tabsNav.on( 'click', '.durotan-size-guide-table-tabs--tab', function() {
		var $el = $( this );
		...
		$tabs.find( '.durotan-size-guide-table-editor[data-tab='+ $el.data( 'tab' ) +']' ).addClass( 'active' ).siblings( '.active' ).removeClass( 'active' );
	} );

	// Edit tab title.
	$tabsNav
		.on( 'click', '.edit-button', function() {
			$( this ).closest( '.durotan-size-guide-table-tabs--tab' ).addClass( 'editting' ).children( 'input' ).removeClass( 'hidden' );
		} )
		.on( 'click', '.confirm-button', function() {
			$( this ).closest( '.durotan-size-guide-table-tabs--tab' ).removeClass( 'editting' ).children( 'input' ).addClass( 'hidden' );
		} )
		.on( 'input', 'input', function() {
			$( this ).siblings( '.durotan-size-guide-table-tabs--tab-text' ).text( this.value );
		} );
	...
	$tabsNav.on( 'click', '.add-new-tab', function() {
		...
		$tabs.find( '.durotan-size-guide-table-editor[data-tab='+ index +']' ).find( '.durotan-size-guide-table' ).durotanEditTable();
	...
	$tabs.on( 'click', '.delete-table', function( event ) {
		...
		var $deletedPanel = $( this ).closest( '.durotan-size-guide-table-editor' ),
	...
	$tabs.on( 'tab_added tab_removed', function() {
		if ( $tabs.children( '.durotan-size-guide-table-editor' ).length <= 1 ) {
...
(function ($, window, i) {
	...
	$.fn.durotanEditTable = function (options) {

		// Settings
		var s = $.extend({
				data: [ ['']],
				tableClass: 'durotan-table-edit',
				jsonData: false,
				headerCols: false,
				maxRows: 999,
				first_row: true,
