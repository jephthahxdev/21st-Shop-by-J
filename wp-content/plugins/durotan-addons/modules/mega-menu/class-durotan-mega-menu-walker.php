...
class Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_badges_text_color = get_post_meta( $item->ID, 'tamm_menu_item_badges_text_color', true );
		$item_content        	= get_post_meta( $item->ID, 'tamm_menu_item_content', true );
		$item_is_mega         	= apply_filters( 'durotan_menu_item_mega', get_post_meta( $item->ID, 'tamm_menu_item_mega', true ), $item->ID );
		$item_mega_width      	= get_post_meta( $item->ID, 'tamm_menu_item_mega_width', true );
		$item_mega_fullwidth  	= get_post_meta( $item->ID, 'tamm_menu_item_mega_fullwidth', true );
		...
		// Container
		$class = array( 'mega-menu-content row' );
		...
		if ( $item_icon_type === 'svg' ) {
			...
			$item_icon_html  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="durotan-svg-icon">' . \Durotan\Icon::sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		...
		$item_badges = '';
		if ( $item_badges_text ) {
			$item_badges_text_color = $item_badges_text_color ? 'style="color:' . $item_badges_text_color . '"' : '';
			$item_badges  = ! empty( $item_badges_text ) ? '<span ' . $item_badges_text_color . ' class="durotan-badges-text">' . wp_kses_post( $item_badges_text ) . '</span> ' : '';
		}
		...
		if ( ! $item_mega_fullwidth ) {
			$inline .= $item_mega_width ? 'width:' . esc_attr( $item_mega_width ) .'; ' : ' ';
		}
		if ( $inline ) {
		...
		if ( $depth == 1 && intval( $this->in_mega ) ) {
			...
			$class_names = ' class="menu-item-mega col-mega-' . $this->column . '"';
		...
		if ( $depth == 1 && intval( $this->in_mega ) ) {
			...
			$item_output = '<a ' . $attributes . '>' . $item_icon_html . $title . $item_badges . '</a>';
