...
class Edit {
	...
	public function scripts( $hook ) {
		...
		wp_register_style( 'durotan-mega-menu', DUROTAN_ADDONS_URL . 'modules/mega-menu/assets/mega-menu.css', array(
			'media-views',
			'wp-color-picker',
		), '20160530' );
		wp_enqueue_style( 'durotan-mega-menu' );

		wp_register_script( 'durotan-mega-menu', DUROTAN_ADDONS_URL . 'modules/mega-menu/assets/mega-menu.js', array(
			'jquery',
			'jquery-ui-resizable',
			'wp-util',
			'backbone',
		...
		wp_enqueue_media();
		wp_enqueue_script( 'durotan-mega-menu' );
	}
            ...
            <div class="tamm-modal media-modal wp-core-ui">
                <button type="button" class="button-link media-modal-close tamm-modal-close">
                    <span class="media-modal-icon"><span
                                class="screen-reader-text"><?php esc_html_e( 'Close', 'durotan' ) ?></span></span>
                </button>
                    ...
                    <div class="tamm-frame-toolbar media-frame-toolbar">
                        <div class="tamm-toolbar media-toolbar">
                            <div class="tamm-toolbar-primary media-toolbar-primary search-form">
                                <button type="button"
                                        class="button tamm-button tamm-button-save media-button button-primary button-large"><?php esc_html_e( 'Save Changes', 'durotan' ) ?></button>
                                <button type="button"
                                        class="button tamm-button tamm-button-cancel media-button button-secondary button-large"><?php esc_html_e( 'Cancel', 'durotan' ) ?></button>
                                <span class="spinner"></span>
                            </div>
                        </div>
                    </div>
	...
	public function templates() {
		...
		foreach ( $templates as $template ) {
			$file = apply_filters( 'tamm_js_template_path', DUROTAN_ADDONS_DIR . 'modules/mega-menu/tmpl/' . $template . '.php', $template );
			?>
	...
	public function save_menu_item_data() {
		...
		foreach ( $data['menu-item'] as $id => $meta ) {
			...
			if ( $i == 0 ) {
				...
				if ( in_array( 'fullwidth', $keys ) ) {
					update_post_meta( $id, 'tamm_menu_item_mega_fullwidth', true );
				} else {
					delete_post_meta( $id, 'tamm_menu_item_mega_fullwidth' );
				}
		...
		do_action( 'durotan_save_menu_item_data', $data );
