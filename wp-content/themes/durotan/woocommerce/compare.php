...
$table_text = get_option( 'yith_woocompare_table_text', esc_html__( 'Compare products', 'durotan' ) );
...
<h1>
    <?php echo ! empty( $localized_table_text ) ? $localized_table_text : '' ?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'durotan' ) ?></a><?php endif; ?>
</h1>
...
<div id="yith-woocompare" class="woocommerce">
    ...
    <table class="compare-list" cellpadding="0" <?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    <thead>
    <tr>
        <th>&nbsp;</th>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'durotan' ) ?></td>
        </tr>
        ...
        <?php foreach ( $fields as $field => $name ) : ?>
            ...
            <tr class="<?php echo esc_attr( $field ) ?>">
                ...
                foreach( $products as $product_id => $product ) :
                    ...
                    $product_class .= ' ' . $product->get_stock_status();
                    ?>
                    ...
                    <td class="<?php echo esc_attr( $product_class ); ?>"><?php
                        switch( $field ) {
                            ...
                            case 'image':
                                ?>
                                <div class="remove">
                                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>"></a>
                                </div>
                                <?php
                                echo '<div class="image-wrap">' . $product->get_image( 'yith-woocompare-image' ) . '</div>';
                                echo '<h4>' . $product->get_name() . '</h4>';
                                echo method_exists( $product, 'get_price_html' ) ? '<div class="price">' . $product->get_price_html() . '</div>' : '';
