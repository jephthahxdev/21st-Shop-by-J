...
echo apply_filters(
	...
	sprintf(
		'<a href="%s" data-quantity="%s" data-product_id="%d" data-variations="%s" class="%s" aria-label="%s" data-text="%s">
			%s
		...
		esc_attr( isset( $args['aria-label'] ) ? $args['aria-label'] :sprintf( __( 'Add %s to the wishlist', 'durotan' ), '&ldquo;' . $product->get_title() . '&rdquo;' ) ),
		__( 'Wishlist', 'durotan' ),
		empty( $args['icon'] ) ? '' : '<span class="wcboost-wishlist-button__icon">' . $args['icon'] . '</span>',
		esc_html( isset( $args['label'] ) ? $args['label'] : __( 'Add to wishlist', 'durotan' ) )
	),
	$args
);
