...
<!-- WISHLIST MOBILE -->
<ul
	class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo ! empty( $show_cb ) ? 'with-checkbox' : ''; ?> <?php echo ! empty( $no_interactions ) ? 'no-interactions' : ''; ?>"
	...
	if ( $wishlist && $wishlist->has_items() ) :
							...
							<?php if ( $show_variation || $show_dateadded || $show_price ) : ?>
								<table class="item-details-table">
									...
									<?php if ( $show_dateadded && $item->get_date_added() ) : ?>
										<tr>
											<td class="labels">
												<?php esc_html_e( 'Added on:', 'durotan' ); ?>
											</td>
											<td class="value">
										...
										<?php if ( $show_quantity || $show_stock_status ) : ?>
											<table class="additional-info">
												<?php if ( $show_quantity ) : ?>
													<tr>
														<td class="labels">
															<?php esc_html_e( 'Quantity:', 'durotan' ); ?>
														</td>
														<td class="value">
												...
												<?php if ( $show_stock_status ) : ?>
													<tr>
														<td class="labels">
															<?php esc_html_e( 'Stock:', 'durotan' ); ?>
														</td>
														<td class="value">
															<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'durotan' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'durotan' ) . '</span>'; ?>
														</td>
													</tr>
												<?php endif ?>
											</table>
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_remove_product || $repeat_remove_button ) : ?>
							<div class="product-remove">
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'durotan' ) ) ); ?>"><?php echo \Durotan\Icon::get_svg( 'close'); ?></a>
							</div>
						<?php endif; ?>
					</div>
				</li>
		...
		<p class="wishlist-empty">
			<?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'durotan' ) ) ); ?>
		</p>
		<?php
	endif;
	...
	if ( ! empty( $page_links ) ) :
		...
		<p class="pagination-row">
			<?php echo ! empty( $page_links ) ? $page_links : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</p>
	<?php endif; ?>
