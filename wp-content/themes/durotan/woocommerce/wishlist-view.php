...
<!-- WISHLIST TABLE -->
<table
	class="shop_table cart wishlist_table wishlist_view traditional responsive <?php echo ! empty( $no_interactions ) ? 'no-interactions' : ''; ?> <?php echo ! empty( $enable_drag_n_drop ) ? 'sortable' : ''; ?> "
	...
	<tr>
		...
		<th class="product-thumbnail">
			<span class="nobr">
				<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product', 'durotan' ), $wishlist ) ); ?>
			</span>
		</th>
		...
		<?php if ( $show_price || $show_price_variations ) : ?>
			...
			<th class="product-price">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Price', 'durotan' ), $wishlist ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_quantity ) : ?>
			...
			<th class="product-quantity">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', __( 'Quantity', 'durotan' ), $wishlist ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_stock_status ) : ?>
			...
			<th class="product-stock-status">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Status', 'durotan' ), $wishlist ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $enable_drag_n_drop ) : ?>
			...
			<th class="product-arrange">
				<span class="nobr">
					<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', __( 'Arrange', 'durotan' ), $wishlist ) ); ?>
				</span>
			</th>
		<?php endif; ?>
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<td class="product-thumbnail">
						...
						<div class="thumbnail">
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
								<?php echo ! empty( $product->get_image() ) ? $product->get_image() : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</a>
						</div>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price">
							<div class="price">
								...
								<?php
								if ( $show_price ) {
									echo ! empty( $item->get_formatted_product_price() ) ? $item->get_formatted_product_price() : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								}

								if ( $show_price_variations ) {
									echo ! empty( $item->get_price_variation() ) ? $item->get_price_variation() : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								}
								?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							...
							<?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html( apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'durotan' ) ) ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html( apply_filters( 'yith_wcwl_in_stock_label', __( 'In Stock', 'durotan' ) ) ) . '</span>'; ?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && $item->get_date_added() ) :
								...
								echo '<span class="dateadded">' . esc_html( sprintf( __( 'Added on: %s', 'durotan' ), $item->get_date_added_formatted() ) ) . '</span>';
							endif;
							?>
							...
							<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
								<?php if ( 'select' === $move_to_another_wishlist_type ) : ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'durotan' ); ?></option>
										<?php
								...
								<?php else : ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										<?php echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'durotan' ) ) ); ?>
									</a>
								<?php endif; ?>
							...
							<?php if ( $repeat_remove_button ) : ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist button" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'durotan' ) ) ); ?>"><?php esc_html_e( 'Remove', 'durotan' ); ?></a>
							<?php endif; ?>
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'durotan' ) ) ); ?>"><?php echo \Durotan\Icon::get_svg( 'close' ); ?></a>
							</div>
						</td>
					<?php endif; ?>
				</tr>
		...
		?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'durotan' ), $wishlist ) ); ?></td>
		</tr>
		<?php
	endif;
	...
	if ( ! empty( $page_links ) ) :
		...
		<tr class="pagination-row wishlist-pagination">
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo ! empty( $page_links ) ? $page_links : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
		</tr>
	<?php endif ?>
	</tbody>
